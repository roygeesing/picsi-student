...
/**
 * Base class for image processing menus
...
public class ImageMenu {
	...
	protected ImageMenu(MenuItem item, TwinView views, MRU mru) {
	...
	public ImageMenu addMenu(String text) {
		MenuItem mi = new MenuItem(m_menu, SWT.CASCADE);
		mi.setText(text);
		return new ImageMenu(mi, m_views, m_mru);
	}
	...
	public boolean findAndRun(String text) {
		for(int i = 0; i < m_menu.getItemCount(); i++) {
			...
			if (menu != null) {
				ImageMenu im = (ImageMenu)menu.getData();
